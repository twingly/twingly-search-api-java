...
class QueryBuilderSpockTest extends Specification {
    ...
    def "should create #expectedQueryString for searchQuery=#sq, location=#loc, lang=#lang, startTime=#st and endTime=#et"() {
        ...
        "sq" | "ua"             | "uk"               | SDF.parse("2017-05-10 01:26:53") | SDF.parse("2017-05-11 01:26:53") || String.format("sq lang:uk start-date:%s end-date:%s location:ua", SDF.format(st), SDF.format(et))
        "sq" | "ua"             | "uk"               | SDF.parse("2017-05-10 01:26:53") | null                              || String.format("sq lang:uk start-date:%s location:ua", SDF.format(st))
        "sq" | "ua"             | "uk"               | null                              | SDF.parse("2017-05-11 01:26:53") || String.format("sq lang:uk end-date:%s location:ua", SDF.format(et))
        ...
        "sq" | Location.Ukraine | Language.Ukrainian | SDF.parse("2017-07-10 01:26:53") | SDF.parse("2017-05-11 01:26:53") || String.format("sq lang:uk start-date:%s location:ua", SDF.format(st))
    }
    ...
    def "should not set end time if it's before start time"() {
        given:
        def startTime = SDF.parse("2016-05-03 15:30:45")
        def endTime = SDF.parse("2016-03-03 15:30:45")
        when:
        def query = QueryBuilder.create("pattern").startTime(startTime).endTime(endTime).build()
        then:
        query.endTime == null
    ...
    def "should create valid query object with all fields set"() {
        ...
        def language = "en"
        def startTime = SDF.parse("2016-04-03 15:30:45")
        def endTime = SDF.parse("2016-05-03 15:30:45")
        when:
